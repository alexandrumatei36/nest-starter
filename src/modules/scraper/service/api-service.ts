...
import { Transaction, TxInExpanded, TxOut } from "../../zenotta/model";
import { DataSource, In, Repository } from "typeorm";
import { countTxnsHashesForAddressesQuery, getTxnsHashesForAddressesQuery } from "../adapter/db/queries";
import { InjectRepository } from "@nestjs/typeorm";

@Injectable()
export class ScraperApiService {
  constructor(private dataSource: DataSource, @InjectRepository(TxInExpanded) private txInExpandedRepository: Repository<TxInExpanded>, @InjectRepository(TxOut) private txOutRepository: Repository<TxOut>) {}

  public async getTransactions(query: GetTransactionsQuery) {
    console.log(query.addresses.split(","))
    ...
    return {
      transactions: txns.map((tx) => {
        return {
          ...
          outs: txOutsMap[tx.hash].map((txOut) => ({ scriptPublicKey: txOut.scriptPublicKey, amount: txOut.amount})),
        }
      }),
      pagination: {
